# Política de Privacidad - SENERGY

**Última actualización:** 20 de enero de 2026

---

## 1. Introducción

En Joaquin Alberto Vilches Montenegro ("Nosotros", "Nuestro"), operadores de la aplicación SENERGY ("la Aplicación"), nos comprometemos a proteger la privacidad y los datos personales de nuestros usuarios ("Usted", "Usuario").

Esta Política de Privacidad describe:
- Qué datos personales recopilamos
- Cómo utilizamos sus datos
- Con quién compartimos sus datos
- Cómo protegemos sus datos
- Sus derechos sobre sus datos

Esta política cumple con:
- **Ley N° 19.628 sobre Protección de la Vida Privada** (Chile)
- **Reglamento General de Protección de Datos (GDPR)** (Unión Europea)
- Mejores prácticas internacionales de privacidad

**Al usar SENERGY, usted acepta las prácticas descritas en esta Política de Privacidad.**

## 2. Responsable del Tratamiento de Datos

**Responsable:**
- Nombre: Joaquin Alberto Vilches Montenegro
- RUT: 20.712.316-1
- Dirección: Villa Colonial Pasaje La Monarquía #573, El Monte, Chile
- Email de contacto: joaquinpersonal4731@gmail.com
- Teléfono: +56 9 9773 9765

## 3. Datos Personales que Recopilamos

### 3.1 Datos Proporcionados Directamente por Usted

**Al registrarse:**
- Nombre completo
- Dirección de correo electrónico
- Contraseña (encriptada, nunca almacenamos contraseñas en texto plano)

**Al usar la Aplicación:**
- Información de medidores eléctricos:
  - Número de medidor
  - Compañía eléctrica
  - Región/ubicación del medidor
  - Tarifa eléctrica configurada

- Lecturas de consumo:
  - Fecha y hora de lectura
  - Valor de lectura (kWh)
  - Notas opcionales
  - Fotografías de medidores (solo usuarios Premium)

- Datos de suscripción:
  - Plan seleccionado (FREE o PREMIUM)
  - Fecha de inicio y expiración de suscripción
  - Historial de pagos

### 3.2 Datos Recopilados Automáticamente

**Datos de uso:**
- Fecha y hora de acceso a la Aplicación
- Funcionalidades utilizadas
- Acciones realizadas (agregar medidor, registrar lectura, etc.)
- Tiempo de uso de la Aplicación

**Datos técnicos:**
- Tipo de dispositivo (iPhone, Android)
- Sistema operativo y versión
- Versión de la Aplicación
- Idioma del dispositivo
- Zona horaria

**Datos de rendimiento:**
- Errores y crashes de la aplicación
- Tiempos de carga
- Métricas de rendimiento

### 3.3 Datos que NO Recopilamos

❌ **NO recopilamos:**
- Ubicación GPS precisa (solo región seleccionada por el usuario)
- Datos de contactos de su teléfono
- Fotos de su galería (solo las que usted suba explícitamente)
- Mensajes de texto o llamadas
- Datos de otras aplicaciones

## 4. Cómo Utilizamos sus Datos

Utilizamos sus datos personales únicamente para los siguientes propósitos legítimos:

### 4.1 Provisión del Servicio (Base legal: Ejecución de contrato)
- Crear y gestionar su cuenta de usuario
- Almacenar sus medidores y lecturas
- Calcular estadísticas de consumo
- Generar gráficos e insights
- Exportar datos (usuarios Premium)
- Enviar notificaciones de la Aplicación

### 4.2 Procesamiento de Pagos (Base legal: Ejecución de contrato)
- Procesar suscripciones Premium
- Gestionar renovaciones y cancelaciones
- Emitir recibos de pago

### 4.3 Mejora del Servicio (Base legal: Interés legítimo)
- Analizar cómo se usa la Aplicación para mejorarla
- Detectar y corregir errores
- Desarrollar nuevas funcionalidades
- Optimizar rendimiento

### 4.4 Comunicación con el Usuario (Base legal: Consentimiento/Interés legítimo)
- Responder a sus consultas de soporte
- Enviar notificaciones sobre cambios en el servicio
- Enviar notificaciones sobre actualizaciones importantes
- Recordatorios de renovación de suscripción

### 4.5 Seguridad y Prevención de Fraude (Base legal: Interés legítimo)
- Proteger contra uso fraudulento
- Detectar y prevenir abusos
- Cumplir con obligaciones legales

### 4.6 Cumplimiento Legal (Base legal: Obligación legal)
- Cumplir con leyes aplicables
- Responder a solicitudes de autoridades competentes

## 5. Base Legal para el Tratamiento (GDPR)

Para usuarios en la Unión Europea, procesamos sus datos bajo las siguientes bases legales:

- **Consentimiento:** Cuando usted acepta esta Política de Privacidad
- **Ejecución de contrato:** Para proporcionar el servicio que solicitó
- **Interés legítimo:** Para mejorar nuestro servicio y prevenir fraude
- **Obligación legal:** Para cumplir con leyes aplicables

## 6. Con Quién Compartimos sus Datos

**NO vendemos, alquilamos ni compartimos sus datos personales con terceros para fines de marketing.**

Compartimos datos solo en las siguientes circunstancias limitadas:

### 6.1 Proveedores de Servicios (Encargados de Tratamiento)

**Google Firebase (Infraestructura y Base de Datos)**
- **Qué datos:** Todos los datos de la Aplicación (autenticación, medidores, lecturas)
- **Propósito:** Almacenamiento y procesamiento de datos
- **Ubicación:** Servidores de Google (multi-región, incluye EE.UU.)
- **Política de privacidad:** https://firebase.google.com/support/privacy
- **Garantías:** Google Cloud cumple con GDPR mediante Cláusulas Contractuales Estándar

**Cloudinary (Almacenamiento de Fotos)**
- **Qué datos:** Fotografías de medidores (solo usuarios Premium)
- **Propósito:** Almacenamiento y entrega de imágenes
- **Ubicación:** Servidores de Cloudinary (multi-región)
- **Política de privacidad:** https://cloudinary.com/privacy
- **Garantías:** Cloudinary cumple con GDPR

### 6.2 Autoridades Legales

Podemos divulgar sus datos si:
- Lo requiere la ley chilena o una orden judicial
- Es necesario para proteger nuestros derechos legales
- Es necesario para prevenir daños a personas o propiedad

### 6.3 Transferencias Internacionales de Datos

Sus datos pueden ser transferidos y procesados fuera de Chile, específicamente:
- **Estados Unidos** (Firebase, Cloudinary)
- **Unión Europea** (Firebase, Cloudinary)

Para transferencias fuera de Chile, utilizamos:
- Cláusulas Contractuales Estándar aprobadas por la UE
- Certificaciones de privacidad (Privacy Shield donde aplique)
- Garantías de seguridad equivalentes a las requeridas en Chile

**Usted tiene derecho a obtener información sobre las garantías aplicables contactándonos.**

### 6.4 Google AdMob (Publicidad)

**IMPORTANTE:** SENERGY muestra anuncios a usuarios del plan GRATUITO mediante Google AdMob.

**Google AdMob (Publicidad para usuarios FREE)**
- **Qué datos:** Google AdMob puede recopilar:
  - ID de publicidad del dispositivo (IDFA en iOS, AAID en Android)
  - Dirección IP
  - Información del dispositivo (modelo, sistema operativo, versión)
  - Datos de uso de la aplicación
  - Datos de ubicación aproximada (basada en IP)
  - Identificadores de anuncios
  - Historial de interacción con anuncios

- **Propósito:**
  - Mostrar anuncios personalizados
  - Medir rendimiento de anuncios
  - Prevenir fraude publicitario
  - Optimizar la experiencia publicitaria

- **Base legal:** Consentimiento e interés legítimo (monetización del servicio gratuito)

- **Ubicación:** Servidores de Google (global, incluye EE.UU.)

- **Política de privacidad de Google:** https://policies.google.com/privacy

- **Política de AdMob:** https://support.google.com/admob/answer/6128543

- **Control del usuario:**
  - **Usuarios Premium NO ven anuncios** y AdMob no recopila datos
  - Usuarios FREE pueden optar por no recibir anuncios personalizados desde:
    - **iOS:** Configuración → Privacidad → Apple Advertising → Desactivar "Anuncios personalizados"
    - **Android:** Configuración → Google → Anuncios → Desactivar "Personalización de anuncios"
  - Puede actualizar a Premium en cualquier momento para eliminar anuncios completamente

**Socios publicitarios de Google AdMob:**
Google puede compartir datos con redes publicitarias asociadas para optimizar anuncios. La lista completa está disponible en:
https://support.google.com/admob/answer/9012903

**Tecnologías de rastreo publicitario:**
- Identificadores de publicidad (reseteables desde configuración del dispositivo)
- Cookies (solo en webviews de anuncios, no en la app principal)
- SDKs de terceros integrados por Google AdMob

**Sus derechos con respecto a publicidad:**
✅ Desactivar personalización de anuncios (seguirá viendo anuncios genéricos)
✅ Resetear su ID de publicidad desde configuración del dispositivo
✅ Actualizar a Premium para eliminar anuncios por completo
✅ Solicitar información sobre datos recopilados por AdMob contactándonos

## 7. Retención de Datos

Conservamos sus datos personales mientras:
- Su cuenta esté activa
- Sea necesario para proporcionar el servicio
- Lo requiera la ley

**Períodos específicos de retención:**

| Tipo de Dato | Período de Retención |
|--------------|---------------------|
| Datos de cuenta | Mientras la cuenta esté activa |
| Lecturas y medidores | Mientras la cuenta esté activa |
| Fotografías | Mientras la cuenta esté activa |
| Datos de suscripción | 7 años (por requisitos fiscales) |
| Logs de seguridad | 6 meses |
| Datos de soporte | 2 años después de la última interacción |

**Al eliminar su cuenta:**
- Eliminaremos permanentemente sus datos personales dentro de 30 días
- Los datos requeridos por ley se conservarán de forma anonimizada
- Las copias de seguridad se eliminarán en el siguiente ciclo (máximo 90 días)

## 8. Seguridad de los Datos

Implementamos medidas técnicas y organizativas para proteger sus datos:

### 8.1 Medidas Técnicas
✅ Encriptación de datos en tránsito (HTTPS/TLS)
✅ Encriptación de contraseñas (bcrypt)
✅ Autenticación segura con Firebase
✅ Firewalls y reglas de seguridad de Firestore
✅ Backups automáticos encriptados
✅ Monitoreo de seguridad 24/7

### 8.2 Medidas Organizativas
✅ Acceso limitado a datos personales (mínimo privilegio)
✅ Auditorías de seguridad regulares
✅ Capacitación en privacidad y seguridad
✅ Políticas de respuesta a incidentes

### 8.3 Responsabilidad del Usuario
Para proteger su cuenta, usted debe:
- Usar una contraseña fuerte y única
- No compartir sus credenciales
- Cerrar sesión en dispositivos compartidos
- Notificarnos inmediatamente de accesos no autorizados

**Sin embargo, ningún método de transmisión o almacenamiento electrónico es 100% seguro. No podemos garantizar seguridad absoluta.**

## 9. Sus Derechos (Ley 19.628 y GDPR)

Usted tiene los siguientes derechos sobre sus datos personales:

### 9.1 Derecho de Acceso
Puede solicitar una copia de todos los datos personales que tenemos sobre usted.

**Cómo ejercerlo:** Envíe un email a joaquinpersonal4731@gmail.com con el asunto "Solicitud de Acceso a Datos"

### 9.2 Derecho de Rectificación
Puede corregir datos inexactos o incompletos.

**Cómo ejercerlo:** Directamente desde la Aplicación (Perfil → Editar) o contactándonos

### 9.3 Derecho de Eliminación ("Derecho al Olvido")
Puede solicitar la eliminación de sus datos.

**Cómo ejercerlo:** Eliminar su cuenta desde la Aplicación o contactarnos

**Excepciones:** Podemos conservar datos si:
- Lo requiere la ley (ej. datos fiscales)
- Es necesario para reclamaciones legales

### 9.4 Derecho de Oposición
Puede oponerse al procesamiento de sus datos para ciertos propósitos (ej. marketing).

**Cómo ejercerlo:** Contactarnos a joaquinpersonal4731@gmail.com

### 9.5 Derecho de Portabilidad
Puede solicitar sus datos en un formato estructurado y legible por máquina.

**Cómo ejercerlo:** Usar la función "Exportar Datos" (usuarios Premium) o contactarnos

### 9.6 Derecho de Limitación del Tratamiento
Puede solicitar limitar el procesamiento de sus datos en ciertas circunstancias.

**Cómo ejercerlo:** Contactarnos a joaquinpersonal4731@gmail.com

### 9.7 Derecho a Revocar Consentimiento
Puede revocar su consentimiento en cualquier momento.

**Cómo ejercerlo:** Eliminar su cuenta o contactarnos

### 9.8 Derecho a Presentar Reclamación
Si considera que vulneramos sus derechos, puede presentar una reclamación ante:

**En Chile:**
- Consejo para la Transparencia: https://www.consejotransparencia.cl
- Email: consultas@consejotransparencia.cl

**En la Unión Europea:**
- Su autoridad de protección de datos nacional

**Respuesta a Solicitudes:**
Responderemos a sus solicitudes dentro de:
- **30 días** (Ley 19.628 de Chile)
- **1 mes** (GDPR, extensible a 3 meses en casos complejos)

## 10. Datos de Menores de Edad

SENERGY está destinada a personas mayores de **18 años**.

**NO recopilamos intencionalmente datos de menores de 18 años.**

Si descubrimos que un menor ha proporcionado datos personales, eliminaremos su cuenta inmediatamente.

Si usted es padre/madre/tutor y cree que su hijo menor ha creado una cuenta, contacte a joaquinpersonal4731@gmail.com.

## 11. Cookies y Tecnologías de Rastreo

SENERGY es una aplicación móvil nativa y **NO utiliza cookies de navegador**.

Sin embargo, utilizamos tecnologías similares:

### 11.1 Identificadores de Dispositivo
- Firebase Analytics utiliza identificadores anónimos para analizar el uso
- Puede optar por no participar en analytics desde la configuración del dispositivo

### 11.2 Almacenamiento Local
- Almacenamos configuraciones localmente en su dispositivo (tema, preferencias)
- Estos datos NO se comparten con terceros

### 11.3 Notificaciones Push
- Si acepta notificaciones, almacenamos un token de dispositivo
- Puede desactivar notificaciones desde la Aplicación o configuración del dispositivo

## 12. Enlaces a Terceros

SENERGY puede contener enlaces a sitios web de terceros (ej. sitios de compañías eléctricas).

**NO somos responsables de las prácticas de privacidad de estos sitios.**

Le recomendamos leer las políticas de privacidad de cualquier sitio que visite.

## 13. Cambios a esta Política de Privacidad

Podemos actualizar esta Política de Privacidad periódicamente.

**Le notificaremos cambios materiales mediante:**
- Notificación in-app
- Email a su dirección registrada
- Aviso en la Aplicación

**Fecha de vigencia:** Los cambios entran en vigor al publicarse, a menos que se indique lo contrario.

**Su uso continuado de SENERGY después de cambios constituye su aceptación de la nueva Política.**

## 14. Contacto y Consultas

Si tiene preguntas sobre esta Política de Privacidad o sobre cómo manejamos sus datos, contáctenos:

**Responsable de Protección de Datos:**
- Email: joaquinpersonal4731@gmail.com
- Teléfono: +56 9 9773 9765
- Dirección: Villa Colonial Pasaje La Monarquía #573, El Monte, Chile

**Tiempo de respuesta:** Responderemos dentro de 5 días hábiles.

---

## Resumen en Lenguaje Simple (No vinculante)

Este resumen es solo para su conveniencia. El documento legal completo es el texto anterior.

### ¿Qué datos recopilamos?
📧 Email, nombre, medidores, lecturas, fotos (Premium)

### ¿Para qué los usamos?
✅ Proporcionar el servicio (guardar lecturas, calcular consumos)
✅ Mejorar la aplicación
✅ Procesar pagos (Premium)

### ¿Con quién compartimos?
👥 Solo con proveedores técnicos necesarios (Firebase, Cloudinary, Google AdMob)
📢 Google AdMob (solo usuarios FREE) - para mostrar anuncios
❌ NUNCA vendemos sus datos

### ¿Dónde se almacenan?
☁️ Firebase (Google Cloud) - servidores globales incluido EE.UU.
📷 Cloudinary - fotos de medidores (Premium)

### ¿Cuánto tiempo guardamos sus datos?
⏱️ Mientras su cuenta esté activa
🗑️ Eliminamos todo en 30 días si elimina su cuenta

### Sus derechos:
✅ Acceder a sus datos
✅ Corregir datos incorrectos
✅ Eliminar su cuenta en cualquier momento
✅ Exportar sus datos
✅ Presentar reclamo ante autoridades
✅ Desactivar anuncios personalizados o actualizar a Premium (sin anuncios)

### Seguridad:
🔒 Encriptación de datos
🔐 Contraseñas protegidas
🛡️ Firebase Security Rules

### Contacto:
📩 joaquinpersonal4731@gmail.com

---

**Última actualización:** 20 de enero de 2026

**Versión:** 1.1

---

## Consentimiento

Al crear una cuenta y usar SENERGY, usted declara que:

✅ Ha leído y comprendido esta Política de Privacidad
✅ Consiente el procesamiento de sus datos personales según lo descrito
✅ Tiene al menos 18 años de edad
✅ Comprende que puede revocar su consentimiento en cualquier momento

Si **NO acepta** esta Política de Privacidad, por favor **NO use SENERGY**.
